...
public class PDFEXPORTDON {
    ...
    public static void exportTableViewToPDF(TableView<Dons> tableView, String imagePath, String filePath, PDDocument document) {
        try {
    ...
    private static float drawTableHeaders(PDPageContentStream contentStream, float xStart, float yStart, float rowHeight, float cellMargin, ObservableList<TableColumn<Dons, ?>> columns) throws IOException {
        float nextX = xStart + cellMargin;
        float nextY = yStart - rowHeight - cellMargin;

        // Draw table headers
        for (TableColumn<Dons, ?> column : columns) {
    ...
    private static void drawTableData(PDPageContentStream contentStream, float xStart, float yStart, float rowHeight, float cellMargin, float rowSpacing, ObservableList<TableColumn<Dons, ?>> columns, ObservableList<Dons> items) throws IOException {
        float nextY = yStart - rowHeight - cellMargin;
        ...
        for (Dons dons : items) {
            ...
            for (TableColumn<Dons, ?> column : columns) {
                Object cellData = column.getCellData(dons); // Use centreDon instead of CentreDon
